 ...
 */
public class ExperiencedGardenerTest {
    private Subscriber subscriber;

    @Before
    public void setUp() throws Exception {
        subscriber = new ExperiencedGardener();
    }

    @Test
    public void informAddsOnlyGardenPressOfHighQuality() throws Exception {
        ...
        assertThat(actualPressCollection, allOf(
                notNullValue(),
                contains(gardenMagazine),
                not(contains(gardenNewspaper)),
                not(contains(javaNewspaper)),
                not(contains(javaMagazine))
        ));
    }
